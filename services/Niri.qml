...
Singleton {
    ...
    property Timer wsAnchorClearTimer: Timer {
        interval: Tokens.anim.durations.normal // ms, adjust as you like
        repeat: false
        onTriggered: {
    ...
    // --- ADD: Hyprland-compatible accessors ---
    readonly property var workspaces: root.allWorkspaces
    readonly property var toplevels: root.windows
    readonly property var focusedWorkspace: root.allWorkspaces[root.focusedWorkspaceIndex] ?? null
    readonly property var activeToplevel: root.focusedWindow
    readonly property int activeWsId: root.focusedWorkspaceIndex + 1
    property bool usingLua: false

    // Monitor changes to focusedWindowId to update lastFocusedWindow
    onFocusedWindowIdChanged: {
        if (focusedWindow) {
            console.log("DEBUG: onFocusedWindowIdChanged called");
    ...
    function updateWorkspaceHasWindows() {
        ...
        for (const ws of root.allWorkspaces) {
            ws.lastIpcObject.windows = root.windows.filter(w => w.workspace_id === ws.id).length;
        }
    }
    ...
    function handleWorkspacesChanged(data) {
        allWorkspaces = [...data.workspaces].sort((a, b) => a.idx - b.idx).map(w => ({
                    id: w.id,
                    name: w.name ?? String(w.id)   // Niri: name is null for numbered workspaces
                    ,
                    monitor: w.output              // Hyprland: workspace.monitor
                    ,
                    is_focused: w.is_focused,
                    is_active: w.is_active,
                    lastIpcObject: {
                        windows: 0
                    }  // backfilled by updateWorkspaceHasWindows
                    ,
                    // Niri-native fields
                    idx: w.idx,
                    output: w.output,
    ...
    function handleWindowsChanged(data) {
        ...
        for (var i = 0; i < newWindows.length; i++) {
            newWindows[i] = root.shapeWindow(newWindows[i]);
        }
    ...
    function shapeWindow(w) {
        ...
        return {
            ...
            lastIpcObject: {
                class: w.app_id,
                fullscreen: w.is_fullscreen ? 2 : 0
            },
    ...
    function handleWindowOpenedOrChanged(data) {
        if (!data.window)
            return;
        var window = root.shapeWindow(data.window);
    ...
    // Map a ShellScreen to a Niri "monitor" object with an activeWorkspace,
    // matching Hyprland's monitorFor() shape so per-monitor code works.
